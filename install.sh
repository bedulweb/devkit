...
# non-interactive-safe PATH (also usable via BASH_ENV=~/.linux-devkit/env)
[ -f "$HOME/.linux-devkit/env" ] && . "$HOME/.linux-devkit/env"
# <<< linux-devkit <<<
EOF
)
  touch "$rc"
...
install_pm2() {
  if have pm2; then ok "pm2 $(pm2 --version 2>/dev/null || echo present)"; return; fi
  log "install pm2 (global via npm)"
  export NVM_DIR="${NVM_DIR:-$HOME/.nvm}"
  # shellcheck disable=SC1091
  [[ -s "$NVM_DIR/nvm.sh" ]] && . "$NVM_DIR/nvm.sh" >/dev/null 2>&1 || true
  if have npm; then
    npm install -g pm2 || { warn "pm2 install failed"; return 0; }
    ok "pm2 $(pm2 --version 2>/dev/null || echo installed)"
  else
    warn "npm missing — skip pm2"
  fi
}
...
write_devkit_env() {
  # ~/.linux-devkit/env — plain sh, idempotent, non-interactive safe.
  # For shells that skip ~/.bashrc (scripts, doppler run, cron):
  #   export BASH_ENV="$HOME/.linux-devkit/env"
  cat > "$DEVKIT_HOME/env" <<'EOF'
# generated by linux-devkit install.sh — safe to source repeatedly
_devkit_path_add() { case ":$PATH:" in *":$1:"*) ;; *) PATH="$1:$PATH" ;; esac; }
_devkit_path_add "$HOME/.local/bin"
[ -x "$HOME/.bun/bin/bun" ] && _devkit_path_add "$HOME/.bun/bin"
if [ -d "$HOME/.nvm/versions/node" ]; then
  _devkit_node_latest="$(ls -d "$HOME"/.nvm/versions/node/v* 2>/dev/null | sort -V | tail -1)"
  [ -n "$_devkit_node_latest" ] && _devkit_path_add "$_devkit_node_latest/bin"
fi
[ -d "$HOME/.local/go/bin" ] && _devkit_path_add "$HOME/.local/go/bin"
GOPATH="${GOPATH:-$HOME/go}"
export GOPATH
[ -d "$GOPATH/bin" ] && _devkit_path_add "$GOPATH/bin"
[ -d "$HOME/.opencode/bin" ] && _devkit_path_add "$HOME/.opencode/bin"
[ -d "$HOME/flutter/bin" ] && _devkit_path_add "$HOME/flutter/bin"
export PATH
